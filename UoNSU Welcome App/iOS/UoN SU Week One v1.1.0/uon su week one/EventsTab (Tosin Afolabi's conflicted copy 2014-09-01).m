...
- (void) viewDidLoad {
    ...
    UIImageView *backgroudView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:TABLEVIEW_BACKGROUND_IMG]];
    [backgroudView setFrame:self.view.frame];
...
- (void) setDefaultMainButton:(NSMutableArray*)buttonArray {
    ...
    for (UIButton *button in buttonArray) {
        ...
        if ([button.titleLabel.text isEqualToString:[NSString stringWithFormat:@"%li",(long)[components day]]]) {
            ...
            mainButton = button;
            [[button layer] setBorderColor:[EVENT_DATE_BUTTON_BACKGROUND_COLOUR_SELECTED CGColor]];
    ...
    if (!mainButton) {
        ...
        mainButton = [buttonArray firstObject];
        [[[buttonArray firstObject] layer] setBorderColor:[EVENT_DATE_BUTTON_BACKGROUND_COLOUR_SELECTED CGColor]];
...
- (void) createEventFromParseData {
    ...
    for (NSString* desc in [description componentsSeparatedByString: @"\n"]) {
        ...
        string = [desc stringByReplacingOccurrencesOfString:@"</p>" withString:@""];
...
- (void) createTableView {
    ...
    tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, SCROLL_VIEW_HEIGHT - SEPARATOR_PADDING_SIZE, window.size.width, self.view.frame.size.height - 56 - 50) style:UITableViewStylePlain];
